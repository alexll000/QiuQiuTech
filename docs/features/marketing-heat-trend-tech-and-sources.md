...
| **起始数据（无爬虫也能看图）** | JSON 种子 + 导入脚本 | `cms/seed/submissions.seed.json`、`scripts/seed/import-submissions-seed.mjs`，见 **`cms/seed/README.md`** |

> 说明：当前抓取路径使用 **原生 `fetch` + 简易 HTML 解析**，对部分强 JS 渲染站点可能不完整；若后续升级为 Playwright，应在 `scripts/crawl/` 另文记录。
